#!/usr/bin/env python3
"""
Train/adapt DeepLab on Lobbe using EVA-CLIP + BLIP pseudo-labels.
...
supervised by EVA+BLIP pseudo-label masks. Validation uses IOSB validation GT
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser("Train DeepLab with Lobbe EVA+BLIP pseudo-labels")
    ...
    ap.add_argument("--pseudo-lbls", type=Path, default=run_root / "pseudo_labels/lobbe_eva_blip/lbls")
    ap.add_argument("--out-dir", type=Path, default=run_root / "segmentation/deeplab_lobbe_eva_blip")
...
def main() -> None:
    ...
    checks = {
        ...
        "train_lbls_eva_blip": train_lbls,
    ...
    print(f"[DATA] EVA+BLIP pseudo-labels: {n_lbls}", flush=True)
    if n_lbls == 0:
        raise RuntimeError("No EVA+BLIP pseudo-labels found.")
    if n_imgs != n_lbls:
    ...
    print("[DONE] EVA+BLIP DeepLab adaptation complete.", flush=True)
